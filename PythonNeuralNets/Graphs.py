...
@tf.function #comment this out if I want to use print statements to investigate the code (same on line 36) 
#@tf.function(reduce_retracing=True) was suggested by Chatgpt it treats shapes more dynamically, but it creates errors with unknown shapes in post-estimation. Also, lowers compilation time but increases execution time
...
@tf.function(reduce_retracing=True, experimental_relax_shapes=True) #options of tf.function suggested by Claude
...
#@tf.function(reduce_retracing=True) #Suggested by ChatGPT here. But time per episode goes from 1.5 seconds to 100 seconds in Climate4.
...
def run_cycle(state_episode):
    ...
    # starting learning phase - needed for DROPOUT layer to become active (Frank: the new keras version does this outomatically and has no longer "set_learning_phase"
...
def run_cycles():
    if "post_init" in dir(Hooks) and Parameters.ckpt.current_episode < 2:
        ...
        print(Parameters.starting_state[:2, :])
    ...
    for i in range(Parameters.N_episodes):    
        #try: #frank: I added this based on Claude, to clear session when memory runs out, but it doesn't work.
        ...
        if Parameters.initialize_each_episode:
            ...
            Hooks.post_init() #FV: i added this line
        else:
        ...
        #except tf.errors.ResourceExhaustedError as e:   #FV: I added this based on Claude, but it runs out of memory anyway, first line does not work.
        #    tf.keras.backend.clear_session()
        #    gc.collect()

        
        if i % 1000 == 0 and i>0 : #Frank: was initially "if i % 10 == 0 :"
            tf.print("Garbage collecting")
            tf.keras.backend.clear_session()  #Frank: ChatGPT comments out because compiling the model takes 20 minutes.However this requires more and more memory
            gc.collect()
