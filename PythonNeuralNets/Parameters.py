...
if os.getenv('OMPI_COMM_WORLD_SIZE'):
    ...
    setattr(sys.modules[__name__], "horovod", True) #this allows another script which imports Parameters, to use Parameters.horovod, which equals True.  
else:
...
#### Configuration setup
@hydra.main(config_path=("config_postprocess" if "USE_CONFIG_FROM_RUN_DIR" in os.environ.keys() else "config"), config_name="config.yaml") # Frank: version_base="1.2" as third argument?
def set_conf(cfg):
    ...
    #more flexible code to read any argument from a given optimizer class and a learning rate schedule (similar on line 286)
    optimizer_config = {k: v for k, v in cfg.optimizer.items() if k not in ['optimizer', 'lr_schedule_boundaries', 'lr_schedule_values', 'learning_rate']}

    #optimizer_config = {k: v for k, v in cfg.optimizer.items() if k != 'optimizer' and k != 'lr_schedule'}  #load all optimizer items, except the name of the optimizer and lr_schedule
    if hasattr(cfg.optimizer, 'lr_schedule_boundaries') and cfg.optimizer.lr_schedule_boundaries:
        boundaries = list(cfg.optimizer.lr_schedule_boundaries)
        values = [v * learning_rate_multiplier for v in list(cfg.optimizer.lr_schedule_values)]
        lr_schedule = tf.keras.optimizers.schedules.PiecewiseConstantDecay(
            boundaries=boundaries,
            values=values
        )
        optimizer_config['learning_rate'] = lr_schedule
    else:
        optimizer_config['learning_rate'] = optimizer_config['learning_rate'] * learning_rate_multiplier

    optim = getattr(tf.keras.optimizers, cfg.optimizer.optimizer)(**optimizer_config) #equivalent to tf.keras.optimizers.Adam(learning_rate=0.01, clipvalue=1.0, weight_decay=1e-4)
    #=optimizer_config['learning_rate'] = optimizer_config.pop('learning_rate') * learning_rate_multiplier #If no LR schedule    
    ...
    def policy(s):
        ...
        for i, pol in enumerate(config_policies):
            if 'activation' in pol.keys():
                ...
                #raw_policy = tf.tensor_scatter_nd_update(raw_policy,[[j,i] for j in range(s.shape[0])],eval(activation_str)(raw_policy[:,i]))   #FV: this is replaced by next 3 lines     
                batch_size = tf.shape(s)[0]
                indices = tf.stack([tf.range(batch_size), tf.fill([batch_size], i)], axis=1)
                raw_policy = tf.tensor_scatter_nd_update(raw_policy, indices, 
    ...
    setattr(sys.modules[__name__], "current_episode", tf.Variable(1)) #this allows another script which imports Parameters, to use Parameters.current_episode, which equals the episode number.
    ...
    if cfg.STARTING_POINT == 'LATEST' and manager.latest_checkpoint:
        ...
        ckpt.restore(manager.latest_checkpoint) #ckpt restore overwrites all optimizer parameters in yaml file with the parameters from the checkpoint. 
    ...
    # Override optimizer parameters after any checkpoint restoration (FV: I added the following 15 lines)
    if cfg.STARTING_POINT != 'NEW':
        # Update individual optimizer attributes, preserving internal state (clipvalue etc.)
        for key, value in cfg.optimizer.items():
            if key not in ['optimizer', 'lr_schedule_boundaries', 'lr_schedule_values', 'learning_rate'] and hasattr(optimizer, key):
        ...
        if hasattr(cfg.optimizer, 'lr_schedule_boundaries') and cfg.optimizer.lr_schedule_boundaries:
            ...
            print(f"Learning Rate schedule with next LR: {lr_schedule(optimizer.iterations).numpy()}")
        else:
            optimizer.learning_rate.assign(cfg.optimizer.learning_rate * learning_rate_multiplier)
            print(f"Constant learning rate updated to: {optimizer.learning_rate.numpy()}")
