...
def update(old_states, at, new_vals):
    i = states.index(at)
    #return tf.tensor_scatter_nd_update(old_states,[[j,i] for j in range(old_states.shape[0])], new_vals) # FV: replaced by next 3 lines due to tf.function(reduce_retracing=True)
    batch_size = tf.shape(old_states)[0]
    indices = tf.stack([tf.range(batch_size), tf.fill([batch_size], i)], axis=1)
    return tf.tensor_scatter_nd_update(old_states, indices, new_vals)
